...
plotCS <- function(d=1:length(agEval), 
  ...
  if(layer_type == "method"){ 
    if(cross_section == "p"){
      axx <- rep(prop_vec,length(m))
      label <- rep(names(agEval[[1]][[1]][[1]])[m],each=length(prop_vec))
      z <- apply(z,2,eval(parse(text=f)))
      
    }
    else if(cross_section == "g"){
      axx <- rep(gap_vec,length(m))
      label <- rep(names(agEval[[1]][[1]][[1]])[m],each=length(gap_vec))
      z <- apply(z,2,eval(parse(text=f)))
    }
...
pp<-ggplot(data.frame(z)) +
  geom_ribbon(aes(x=prop_vec, ymin=apply(z,1,min), ymax=apply(z,1,max)), fill = "pink") +
  geom_line(aes(x=prop_vec, y=apply(z,1,median)), col = "red") +
  ggtitle("Collapsing Gap Width") + 
  theme_classic()

pp

pg<-ggplot(data.frame(z)) +
  geom_ribbon(aes(x=gap_vec, ymin=apply(z,2,min), ymax=apply(z,2,max)), fill = "lightblue") +
  geom_line(aes(x=gap_vec, y=apply(z,2,median)), col = "red") +
  ggtitle("Collapsing Proportion Missing") + 
  theme_classic()
