...
plot.D4X.MAPE <- ggplot() + 
  geom_line(aes(x = seq(0.01,0.07, length.out = length(mean_MAPE)), y = mean_MAPE), col = "red") +
      geom_ribbon(aes(x = seq(0.01,0.07, length.out = length(mean_MAPE)), 
                    ymin = mean_q2.5MAPE,
                    ymax = mean_q97.5MAPE),
                alpha = 0.1)  + 
  theme_minimal() + 
  labs(x = "minimum frequency", y = "mean performance", title = "MAPE")

# What the hell!? D18 is weird!
