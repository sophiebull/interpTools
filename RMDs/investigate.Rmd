...
plot.D4X.NRMSD
dev.off()
save(pMats.D4X, file = "pMats.D4X.rda")
save(ag.D4X, file = "ag.D4X.rda")
```
...
```{r}
OriginalData.shift.2 <- load_obj("/home/sophie/sophTools/files/investigate/shift2/OriginalData.shift.2.rda")
GappyData.shift.2 <- load_obj("/home/sophie/sophTools/files/investigate/shift2/GappyData.shift.2.rda")
IntData.shift.2 <- load_obj("/home/sophie/sophTools/files/investigate/shift2/IntData.shift.2.rda")
...
```{r}
# Surface plots
ag.unshift <- load_obj("/home/sophie/sophTools/files/new_1/new.ag.rda")
ag.shift.2 <- load_obj("/home/sophie/sophTools/files/investigate/shift2/agEval.shift.2.rda")

crit = "NRMSD"
plotList.unshift <- plotSurface(agEval = ag.unshift, m = "HWI", d = 4, highlight = 4, highlight_colour = "skyblue", layer_type = "dataset", crit = crit)
plotList.shift.2 <- plotSurface(agEval = ag.shift.2, m = "HWI", d = 1, highlight = 1, layer_type = "dataset", crit = crit)

p <- plotList.unshift[[crit]][["HWI"]]
pshift2 <- plotList.shift.2[[crit]][["HWI"]]
...
```{r}
# Heatmaps
# need to scale heatmaps so colour runs across both. right now they are individually scaled.
# Find the range across both. 

crit = "NRMSD"
zlist_unshift <-  compileMatrix(ag.unshift)[["median"]]
zlist_shift2 <- compileMatrix(ag.shift.2)[["median"]]
    
rownames(zlist_unshift[[crit]][["HWI"]][[4]]) <- gsub("p","",rownames(zlist_unshift[[crit]][["HWI"]][[4]]), fixed = TRUE)
colnames(zlist_unshift[[crit]][["HWI"]][[4]]) <- gsub("g","",colnames(zlist_unshift[[crit]][["HWI"]][[4]]), fixed = TRUE)

rownames(zlist_shift2[[crit]][["HWI"]][[1]]) <- gsub("p","",rownames(zlist_shift2[[crit]][["HWI"]][[1]]), fixed = TRUE)
colnames(zlist_shift2[[crit]][["HWI"]][[1]]) <- gsub("g","",colnames(zlist_shift2[[crit]][["HWI"]][[1]]), fixed = TRUE)
    
plott.unshift <- melt(zlist_unshift[[crit]][["HWI"]][[4]])
colnames(plott.unshift) <- c("p","g", "value")

plott.shift2 <- melt(zlist_shift2[[crit]][["HWI"]][[1]])
colnames(plott.shift2) <- c("p","g", "value")

rngg <- range(plott.shift2$value,plott.unshift$value)

map.unshift <- ggplot(plott.unshift, aes(as.factor(p), as.factor(g), fill = value)) + geom_tile() + 
          scale_fill_gradientn(colours = col, values = c(0,1),
                               limits = rngg) +
          
          labs(x = "proportion missing", y = "gap width", fill = crit, title = "unshifted") + 
          theme_minimal() + 
...
map.shift2 <- ggplot(plott.shift2, aes(as.factor(p), as.factor(g), fill = value)) + geom_tile() + 
          scale_fill_gradientn(colours = col, values = c(0,1),
                               limits = rngg) +
          
          labs(x = "proportion missing", y = "gap width", fill = crit, title = "shifted") + 
          theme_minimal() + 
...
dumPlot <- ggplot(plott.unshift, aes(as.factor(p), as.factor(g), fill = value)) + geom_tile() + 
      scale_fill_gradientn(colours = col, values = c(0,1),
                           limits = rngg) +
...
plotWindow <- grid.arrange(map.unshift,map.shift2, ncol = 1)

setwd("/home/sophie/sophTools/files/investigate/shift2")
png(paste0(crit,"_heatmap_shift2.png"), width = 400, height = 1000)
grid.arrange(plotWindow,myLegend, ncol = 1, heights = c(20,2))
dev.off()
```
