...
```{r examplecnt, eval = FALSE}
parInterpolate(GappyList = GappyData[[1]], FUN_CALL = c("plus(x = ", "minus(x = "))
```
Additional built-in functions can be specified with `methods`.

To generate the toy data found in the package files, we will interpolate using the Linear Interpolator (LI), the Nearest Neighbor (NN), and the Kalman ARIMA Filter (KAF).
