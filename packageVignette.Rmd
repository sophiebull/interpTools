...
The intended use of this R package is to provide a set of tools for the systematic testing of time series interpolators in light of changing the __features of the data__ and the __gap structure__. The basic framework is as follows:
...
```{r xt, cache = TRUE, warnings = FALSE}
...
The original time series are saved as `~/toyData/toy_OriginalData.rda`.  
...
The next step is to interpolate the list of gappy data using `parInterpolate()`.  The user can choose from a list of 18 algorithms pulled from external packages, shown in the table below.
...
A vector of `methods` of length $M$ can be defined to facilitate a comparative assessment between algorithms later, if so desired. Should the user decide to pass in their own function via `FUN_CALL`, they must ensure that the output the function is a single numeric vector. The interpolations will occur in parallel over the $K$ index, where the number of cores to use can be defined via `num_cores` (the default is to use as many as are available, as determined by `parallel::detectCores()`).

Here, we interpolate using the Linear Interpolator (LI), the Exponential Weighted Moving Average (EWMA), and the Kalman ARIMA Filter (KAF).

```{r parInterpolate, cache = TRUE}
# Using a loop to interpolate over the five sets of gappy data
D <- length(GappyData)
methods <- c("LI", "EWMA", "KAF")

IntData <- list()

for(d in 1:D){
  IntData[[d]] <- parInterpolate(GappyList = GappyData[[d]], methods = methods)
...
Considering the dimensionality of the data and complexity of some interpolation algorithms (especially that of the Hybrid Wiener Interpolator), the system requirements for this number of computations can be quite large. The function `splitData()` can be used to "break apart" the gappy data at the $K$-level into more manageable chunks (the number of sets specified by the `split` argument) prior to being passed into `parInterpolate()`.
...
The split data is saved as `~/toyData/toy_GappyData_split.rda`.  
...
for(d in 1:D){
  for(n in 1:split){
    IntData_split <- parInterpolate(GappyList = GappyData_split[[d]][[n]], methods = methods)
    filename <- paste0("D",d,"_",letters[n],".rda")
    setwd(paste0("/home/sophie/sophTools/toyData/toy_IntData_split/D",d))
...
IntData_stitch1 <- stitchData("/home/sophie/sophTools/toyData/toy_IntData_split/D1")
IntData_stitch2 <- stitchData("/home/sophie/sophTools/toyData/toy_IntData_split/D2")
IntData_stitch3 <- stitchData("/home/sophie/sophTools/toyData/toy_IntData_split/D3")
IntData_stitch4 <- stitchData("/home/sophie/sophTools/toyData/toy_IntData_split/D4")
IntData_stitch5 <- stitchData("/home/sophie/sophTools/toyData/toy_IntData_split/D5")
...
```{r plotXt}
plotXt(d = 1, simData = xt, cptwise = F)
```
...
Below is an example of a surface plot, layered by __method__, for the performance of the Linear Interpolation (LI, highlighted), Exponential Weighted Moving Average (EWMA), and Kalman Filter (KAF) on the first dataset according to the Mean Squared Error (MSE) criterion, where $f(p,g)$ represents the __median__ value:

```{r plotSurface_m, cache = TRUE}
plotSurface(d = 1, 
            m = c("LI","EWMA","KAF"), 
            layer_type = "method", 
            crit = "MSE", 
...
The function `multiplot()` allows the user to combine multiple surface plot objects into one grid to add another dimension for comparison. The following is an example of a set of surface plots describing datasets 1-5 of the method-layered plots.  

```{r multiSurface_m, cache = TRUE}
multiSurface(d = 1:5, 
             agEval = agEval, 
             layer_type = "method", 
             m = c("LI","EWMA","KAF"), 
...
```{r heatmapGrid2, cache = TRUE}
heatmapGrid2(d = c(1,3), 
             agEval = agEval, 
             f = "median", 
             crit = c("MSE"), 
             m = "LI")
...
```{r multiHeatmap_crit, cache = TRUE}
...
```{r multiHeatmap_method, cache = TRUE}
...
Below is an example of a $g$ cross-section plot of dataset 1, where each ribbon represents the sampling distribution $\left[ Q_{2.5\%}, \text{median}, Q_{97.5\%} \right]$ of performance, according to the Mean Squared Error (MSE) criterion, of interpolation method (LI, EWMA, KAF) with respect to gap width, across all values of $p$. 

```{r plotCS_g, cache = TRUE}
plotCS(d = 1,
       m = c("LI","EWMA","KAF"),
       crit = "MSE", 
       agEval = agEval, 
...
Below is an example of a $p$ cross-section plot of dataset 1, where each ribbon represents the sampling distribution $\left[ Q_{2.5\%}, \text{median}, Q_{97.5\%} \right]$ of performance, according to the Mean Squared Error (MSE) criterion, of interpolation method (LI, EWMA, KAF) with respect to proportion missing, across all values of $g$. 

```{r plotCS_p, cache = TRUE}
plotCS(d = 1,
       m = c("LI","EWMA","KAF"),
       crit = "MSE", 
       agEval = agEval, 
...
### Uncollapsed
The collapsed cross section plots can be further deconstructed using `plotCS_un()` such that the performance can be assessed with respect to changes in one variable ($p$ or $g$) across __each individual value of the other__.  This will give insight into whether there are specific combinations of ($p,g$) for which the performance of a particular method is particularly sensitive. Each ribbon depicts the distribution of a metric across the $K$ simulations, where the upper and lower bounds represent the 2.5% and 97.5% quantiles, and the interior line is the median value.  The reader should note that this is the __same collection of medians__ used in the construction of the surface plot.  Save for producing an overcrowded and hard-to-read data visualization, we can imagine assigning the collection of error bars to each ($p,g$) coordinate point on the corresponding surface plot.

Below is an example of a $p$ cross-section plot of dataset 1, where each ribbon represents the sampling distribution $\left[ Q_{2.5\%}, \text{median}, Q_{97.5\%} \right]$  of performance, according to the Mean Squared Error (MSE) criterion, of interpolation method (LI, EWMA, KAF) with respect to each value of $g$. 

```{r plotCS_un_p, cache = TRUE}
plotCS_un(d = 1,
          agEval = agEval,
          cross_section = "p", 
          crit = "MSE", 
...
The output is a list of tables of values.
...
### Skewness of performance criteria
...
The `plotSkew()` function can be used to plot the distributions of the skewness values for the chosen performance metrics.  The user can toggle the `symmetric` argument to view subsets of skewness distributions: symmetric only, asymmetric only, or the full set. 

```{r plotSkew_plot, eval = FALSE}
plotSkew(agEval = agEval, cptwise=T, symmetric= NULL)
```
...
The `output` argument can be changed such that a table of values is shown instead:
```{r plotSkew_tab, eval = FALSE}
...
### Surface Plots 
As a supplement to the surface plots, the user can easily retrieve the raw data in table-form using the `compileMatrix()` function.

```{r compileMatrix, cache = TRUE, eval = FALSE}
...
The `ztable()` function provides a compact view of the sampling distribution of the data, for any $d$, with respect to the 2.5% quantile, the median, and the 97.5% quantile.  For example, that of the Linear Interpolation method, on the first dataset according to the Mean Squared Error (MSE) criterion:

```{r ztable, cache = TRUE, eval = FALSE}
ztable(agEval = agEval, d = 1 , crit = "MSE", m = "LI", combine = T)
```

The table is produced in $\LaTeX$ format.
